...
const AdminApp: React.FC = () => {
  ...
  const [conversationsCount, setConversationsCount] = useState({ total: 0, today: 0 })
  ...
  const modulePermissions = {
    ...
    'analytics': [
      ...
      { id: 'analytics_realtime', name: 'Real-time Data', description: 'View real-time analytics' }
    ],
    'faq': [
      { id: 'faq_view', name: 'View FAQs', description: 'Read FAQ articles' },
      { id: 'faq_create', name: 'Create FAQs', description: 'Add new FAQ articles' },
      { id: 'faq_edit', name: 'Edit FAQs', description: 'Modify existing FAQs' },
      { id: 'faq_delete', name: 'Delete FAQs', description: 'Remove FAQ articles' },
      { id: 'faq_categories', name: 'Manage Categories', description: 'Organize FAQ categories' }
    ],
    'tickets': [
      { id: 'tickets_view', name: 'View Tickets', description: 'See support tickets' },
      ...
      { id: 'tickets_priority', name: 'Set Priority', description: 'Change ticket priority' },
      { id: 'tickets_escalate', name: 'Escalate Tickets', description: 'Escalate to higher level' }
    ],
    'livechat': [
      { id: 'livechat_view', name: 'View Live Chat', description: 'Access live chat interface' },
      { id: 'livechat_respond', name: 'Respond to Chats', description: 'Reply to customer chats' },
  ...
  const hasAccess = (tabId: string) => {
    ...
    // Check if user is main admin from venturing.db
    const isMainAdmin = currentUser.is_admin || currentUser.user_type === 'admin' || currentUser.role_name === 'Super Admin'
    
    // If user is main admin, give full access
    if (isMainAdmin) {
      return true
    }
    ...
    // For any user with role, check permissions from user_management.db
  ...
  // Check specific permission for features
  ...
  // Check if user has any tab access
  const hasAnyTabAccess = () => {
    if (!currentUser) {
      return false
    }
    ...
    // For any user with role, check if they have any permissions
    if (currentUser.role_name && userPermissions.length > 0) {
      return true
    }
    
    // Fallback: check if user has any permissions
  ...
  // Fetch user permissions when currentUser changes and is authenticated
  useEffect(() => {
    if (isAuthenticated && currentUser && currentUser.id) {
      setPermissionsTimeout(false)
      fetchUserPermissions()
      
      // Set a timeout to prevent infinite loading
      const timeout = setTimeout(() => {
        if (permissionsLoading) {
          // console.log('Permissions loading timeout - showing access denied')
  ...
  const updateAnalyticsData = (newData: any) => {
    setAnalyticsData(prevData => {
      const newActivities = (newData.realTimeActivity || []).map((activity: any, index: number) => ({
        ...activity,
        id: activity.id || `activity_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 9)}`
      }))
      ...
      if (!response.ok) {
        ...
        if (userData) {
          try {
            ...
            const refreshResponse = await fetch('http://localhost:8000/auth/login', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({
                ...
                password: 'admin123' // This should be stored securely or use refresh token
              })
            })
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const conversationsCountResponse = await fetch('http://localhost:8000/reports/conversations/count', {
        headers
      })
      if (conversationsCountResponse.ok) {
        const conversationsCountData = await conversationsCountResponse.json()
        setConversationsCount(conversationsCountData)
      } else {
        console.error('Conversations Count API error:', conversationsCountResponse.status)
      }
  ...
  const downloadConversations = async () => {
    try {
      ...
      // Starting download
      const response = await fetch('http://localhost:8000/reports/conversations/download', {
        method: 'GET',
        headers: {
          ...headers,
      ...
      if (response.ok) {
        ...
        a.href = url
        a.download = `chat_conversations_${new Date().toISOString().split('T')[0]}.csv`
      ...
      alert(`Error downloading conversations: ${error.message}`)
    }
  }
  ...
  const downloadAnalytics = async () => {
    try {
      ...
      // Starting analytics download
      const response = await fetch('http://localhost:8000/reports/analytics/download', {
        method: 'GET',
        headers: {
          ...headers,
          'Accept': 'text/csv,application/csv',
      ...
      if (response.ok) {
        ...
        // Analytics download initiated successfully
      } else {
        const errorText = await response.text()
        console.error('Analytics download failed:', response.status, errorText)
        alert(`Analytics download failed: ${response.status} - ${errorText}`)
      }
    } catch (error) {
  ...
  const fetchSubcategories = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch subcategories:', response.status, errorData)
      }
    } catch (error) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (showDownloadDropdown && !target.closest('[data-dropdown]')) {
  ...
  // Fetch user permissions when currentUser changes
  useEffect(() => {
  ...
  const fetchUserPermissions = async (retryCount = 0) => {
    ...
    if (!currentUser || !currentUser.id) {
      // console.log('No currentUser or user ID')
      setUserPermissions([])
      setPermissionsLoading(false)
      return
    }
    
    // console.log('Fetching permissions for user:', currentUser.username, 'ID:', currentUser.id, retryCount > 0 ? `(retry ${retryCount})` : '')
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/users/${currentUser.id}/permissions`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      ...
      if (response.ok) {
        ...
        setPermissionsTimeout(false) // Reset timeout on success
      } else if (response.status === 404) {
        // User not found in user_management.db (admin user from main DB)
        // console.log('User not found in user_management.db')
        setUserPermissions([]) // Empty permissions will trigger fallback logic
        setPermissionsTimeout(false) // Reset timeout on success
      } else {
        console.error('Error fetching user permissions:', response.status)
        setUserPermissions([])
        
        // Retry on server errors (5xx) up to 2 times
        if (response.status >= 500 && retryCount < 2) {
          // console.log(`Retrying permissions fetch in 2 seconds... (attempt ${retryCount + 1})`)
          setTimeout(() => {
  ...
  const handleManagePermissions = async (role: any) => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        const tabPermissionsMap: Record<string, boolean> = {}
        
        if (data.permissions && Array.isArray(data.permissions)) {
          // Convert permission IDs to tab access
          const permissionNames = data.permissions.map((p: any) => p.name)
          
          // Map permissions to tabs
          Object.keys(tabPermissions).forEach(tabId => {
            const requiredPermission = tabPermissions[tabId]
            const hasAccess = permissionNames.includes(requiredPermission)
            tabPermissionsMap[tabId] = hasAccess
          })
        }
        
        setRolePermissions(tabPermissionsMap)
      } else {
        console.error('Failed to load role permissions:', response.status)
        setRolePermissions({})
      }
      
      setShowPermissionModal(true)
    } catch (error) {
      console.error('Error in handleManagePermissions:', error)
      setShowPermissionModal(true) // Still show modal even if permissions fail to load
    }
  }

  const handleModulePermissionClick = async (moduleId: string) => {
    console.log('Module clicked:', moduleId)
    setSelectedModule(moduleId)
    
    // Load current permissions for this module
    try {
      const token = localStorage.getItem('admin_token')
      if (!token || !editingRole) return
      
      const response = await fetch(`http://localhost:8000/api/users/roles/${editingRole.id}/permissions`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      ...
      if (response.ok) {
        ...
        // Initialize permission states for this module
        const modulePerms = modulePermissions[moduleId as keyof typeof modulePermissions] || []
        const newStates: Record<string, boolean> = {}
        
        modulePerms.forEach(perm => {
          const hasPermission = currentPermissions.some((p: any) => p.name === perm.id)
          newStates[perm.id] = hasPermission
        })
        
        setModulePermissionStates(newStates)
      }
    } catch (error) {
  ...
  const handleModulePermissionChange = (permissionId: string, checked: boolean) => {
  ...
  const saveModulePermissions = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter out permissions for the current module
        const otherModulePermissions = currentPermissions.filter((p: any) => {
          const modulePerms = modulePermissions[selectedModule as keyof typeof modulePermissions] || []
          return !modulePerms.some(mp => mp.id === p.name)
        })
        
        // Get all permissions to convert names to IDs
        const allPermissionsResponse = await fetch('http://localhost:8000/api/users/permissions', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
        ...
        const selectedPermissionNames = Object.entries(modulePermissionStates)
        ...
        // Combine with other module permissions
        const allPermissionIds = [
          ...otherModulePermissions.map((p: any) => p.id),
        ...
        const updateResponse = await fetch(`http://localhost:8000/api/users/roles/${editingRole.id}/permissions`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          },
          body: JSON.stringify(allPermissionIds)
        })
        ...
        if (updateResponse.ok) {
          ...
          setSelectedModule('')
          // Refresh the main permissions modal
          await handleManagePermissions(editingRole)
        } else {
          const errorText = await updateResponse.text()
          console.error('Error response:', errorText)
          alert('Error saving permissions: ' + errorText)
        }
      }
    } catch (error) {
  ...
  const generateMockActivity = () => {
    ...
    for (let i = 0; i < 15; i++) {
      activities.push({
        id: `activity_${baseTime}_${i}_${Math.random().toString(36).substr(2, 9)}`,
        type: activityTypes[Math.floor(Math.random() * activityTypes.length)],
        timestamp: new Date(baseTime - Math.random() * 300000).toLocaleTimeString(),
  ...
  const deleteFAQ = async (faqId: string) => {
    // Ask user to choose between soft delete and hard delete
    const deleteType = confirm('Choose delete type:\n\nOK = Soft Delete (can be recovered)\nCancel = Hard Delete (permanent)')
    ...
    try {
      ...
      const endpoint = deleteType 
        ...
        : `http://localhost:8000/admin/faqs/${faqId}/permanent`
      ...
      if (response.ok) {
        ...
        // Also refresh the FAQ list from server to ensure consistency
        try {
          const refreshResponse = await fetch('http://localhost:8000/admin/faqs', {
            headers: {
              'Content-Type': 'application/json',
            }
          })
          if (refreshResponse.ok) {
            ...
            console.error('❌ Failed to refresh FAQ list:', refreshResponse.status)
          }
        } catch (refreshError) {
          console.error('❌ Error refreshing FAQ list:', refreshError)
        }
        
        const deleteMessage = deleteType 
          ? '✅ FAQ soft deleted successfully! (Can be recovered)'
          : '✅ FAQ permanently deleted successfully! (Cannot be recovered)'
        alert(deleteMessage)
      } else {
        const errorData = await response.json()
      ...
      alert(`❌ Error deleting FAQ: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: '14px',
                  cursor: 'pointer',
                  opacity: (!newSubcategory.name.trim() || !newSubcategory.description.trim()) ? 0.5 : 1
                }}
              >
                Update Subcategory
              </button>
  ...
  const renderTabContent = () => {
      ...
      return (
        <div style={{
          ...
          <p style={{
            ...
            Please wait while we load your access permissions.
          </p>
        </div>
      )
    }
      ...
      return (
        <div style={{
          ...
          <p style={{
            ...
            You don't have permission to access any modules in the admin dashboard.
          </p>
          <p style={{
            fontSize: '14px',
            color: '#9ca3af',
            ...
            Please contact your administrator to get the necessary permissions assigned to your account.
          </p>
          <button
    ...
    if (!hasAccess(activeTab)) {
      // Show loading state while permissions are being fetched (and not timed out)
      if (permissionsLoading && !permissionsTimeout) {
        return (
          <div style={{
            display: 'flex',
            flexDirection: 'column',
      ...
      return (
        <div style={{
          ...
          <p style={{
            ...
            You don't have permission to access this module. Please contact your administrator if you need access.
          </p>
        </div>
      )
    }
      ...
      <div style={{ 
        ...
        <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
          {analyticsData.realTimeActivity.map((activity, index) => (
            <div key={`${activity.id}_${index}`} style={{ 
              display: 'flex', 
              alignItems: 'center', 
              padding: '12px 0',
    ...
    <div style={{ padding: '24px 16px' }}>
      <div style={{ 
        ...
        <div style={{ display: 'flex', gap: '12px', position: 'relative' }}>
          {(hasPermission('reports_download_conversations') || hasPermission('reports_download_analytics')) && (
            ...
            {showDownloadDropdown && (
              <div style={{
                ...
                {hasPermission('reports_download_conversations') && (
                  <button
                    onClick={() => {
                  ...
                  >
                    ...
                    <div>
                      <div style={{ fontWeight: '600', marginBottom: '2px' }}>Chat Conversations</div>
                      <div style={{ fontSize: '12px', color: '#6b7280' }}>Download in Excel format</div>
                    </div>
                  </button>
                )}
                {hasPermission('reports_download_analytics') && (
                  <button
                    onClick={() => {
                      downloadAnalytics()
                      setShowDownloadDropdown(false)
                  ...
                  >
                    ...
                    <div>
                      <div style={{ fontWeight: '600', marginBottom: '2px' }}>Analytics Report</div>
                      <div style={{ fontSize: '12px', color: '#6b7280' }}>Download in Excel format</div>
                    </div>
                  </button>
                )}
          ...
          {hasPermission('reports_generate_faq_report') && (
            <button
              onClick={() => {
      ...
      {hasPermission('reports_view_summary') && (
        <div style={{ 
          display: 'grid', 
          gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', 
          gap: '20px',
      ...
      {hasPermission('reports_faq_performance') && (
        <div style={{ 
          backgroundColor: 'white', 
          borderRadius: '12px', 
          padding: '24px', 
      ...
      {hasPermission('reports_category_performance') && (
        <div style={{ 
          backgroundColor: 'white', 
          borderRadius: '12px', 
          padding: '24px', 
            ...
            <form onSubmit={async (e) => {
              ...
              <div style={{ marginBottom: '32px' }}>
                ...
                <p style={{ 
                  ...
                  💡 Select the permissions this role should have. You can always modify them later.
                </p>
              </div>
            ...
            <div style={{ 
              ...
              <div>
                ...
                <p style={{ 
                  ...
                  Configure detailed permissions for {editingRole?.name} role
                </p>
              </div>
              <button
                onClick={() => {
